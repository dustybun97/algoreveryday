...
// หาคะแนนเฉลี่ย IMDB Rating ของหนังประเภท Drama โดยใช้ Built In

// ผลลัพธ์จะเป็น : 8.44 ( เป็น Number และมีจุดทศนิยมแค่ 2 ตัว ) (edited)
// .filter((word) => word !== " ")

//Algor
//filter หา movies ที่เป็น genres "Drama"
//.map เอา imdb มารวมกัน

//task
//1.filter เพื่อแสดงแค่ genres "Drama" และมีคะแนน (ไม่เป็น N/A)
//2.ดึงคะแนนจาก Drama มารวมกัน
//3.นำคะแนนทั้งหมดมาหาค่า avg
//4.ปัดทศนิยม

let findDrama;
findDrama = movies.filter(
  (movie) => movie.genres.includes("Drama") && movie.imdbRating !== "N/A"
);
console.log(findDrama);
let findScore;
findScore = findDrama.reduce((acc, cur) => acc + parseFloat(cur.imdbRating), 0);
console.log(findScore);
let findFinalScore;
findFinalScore = (findScore / findDrama.length).toFixed(2);
console.log(`Total Score for Drama genres is : ${findFinalScore} !!!!`);
